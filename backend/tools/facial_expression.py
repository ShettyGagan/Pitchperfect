...
@tool(
    name="facial_expression_analyzer",
    description="Analyzes facial expressions in a video to detect emotions and engagement metrics.",
    show_result=True,
    stop_after_tool_call=True,
...
def analyze_facial_expression(video_path: str) -> str:
    """
    Analyse facial expressions frame by frame.
    ...
    smile_count = 0
    frame_count = 0
    fps = cap.get(cv2.CAP_PROP_FPS) or 25  # fallback if fps undetectable
    frame_interval = 5
    ...
    while cap.isOpened():
        ...
        for face_landmarks in results.multi_face_landmarks:
            landmarks = face_landmarks.landmark
            h, w, _ = frame.shape
            lc = [(int(lm.x * w), int(lm.y * h)) for lm in landmarks]
            ...
            try:
                ...
                emotion = analysis[0]["dominant_emotion"]
                print(f"DeepFace emotion: {emotion} at frame {frame_count}")
                ...
                print(f"Error analysing frame {frame_count}: {e}")
                continue
            ...
            left_opening = np.linalg.norm(
                np.array(left_upper) - np.array(left_lower)
            )
            right_opening = np.linalg.norm(
                np.array(right_upper) - np.array(right_lower)
            )

            if (left_opening + right_opening) / 2 > 5:
                eye_contact_count += 1

    cap.release()
    face_mesh.close()

    total_processed = max(frame_count // frame_interval, 1)

    result = {
        "emotion_timeline": emotion_timeline,
        "engagement_metrics": {
            "eye_contact_frequency": round(eye_contact_count / total_processed, 4),
            "smile_frequency": round(smile_count / total_processed, 4),
        },
    }
